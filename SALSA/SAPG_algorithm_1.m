...
%}
function [theta_EB, tau_EB, results] = SAPG_algorithm_1(y, op)
    tic;
    %% Setup     
    ...
    if(warmupSteps > 0)
        ...
        for ii = 2:warmupSteps
            ...
            X_wu =  X_wu + gamma* (proxGX_wu-X_wu)/lambda -gamma*gradF(X_wu, fix_tau) + sqrt(2*gamma)*randn(size(X_wu));
            proxGX_wu = proxG(X_wu,lambda,fix_theta);
            %Save current state to monitor convergence
            logPiTrace_WU(ii) = logPi(X_wu,fix_theta, fix_tau);
            %Display Progress
            if mod(ii, round(op.warmup / 100)) == 0
                fprintf('\b\b\b\b%2d%% \t %d\n', round(ii / (op.warmup) * 100), ii);
            end
        end
        results.logPiTrace_WU=logPiTrace_WU;
    end
    %% Run SAPG algorithm 1 to estimate theta and eta
    theta(total_iter)=0;
    theta(1)=op.th_init;
    eta(total_iter)=0;
    eta(1)=eta_init;
    ...
    tol_taus(total_iter) = 0;
    
    %% mean Value
    mean_taus(total_iter - op.burnIn) = 0;
    ...
    logPiTraceX(1)=logPi(X,theta(1), tau(1));
    ...
    for ii = 2:total_iter 
        %% Sample from posterior with MYULA:
        Z = randn(size(X));
        X =  X + gamma* (proxGX-X)/lambda - gamma*gradF(X, tau(ii-1)) + sqrt(2*gamma)*Z;
        ...
        %% update tau
        to = tau(ii-1) -  delta(ii) * gradF_to(X, tau(ii-1));
        tau(ii) = min(max(to,min_tau),max_tau);
        
        
        %% Save current state to monitor convergence
        logPiTraceX(ii) = logPi(X,theta(ii-1), to);
        gX(ii-1) = g(X);

        %% Display Progress
        if mod(ii, round(total_iter / 100)) == 0
            fprintf('\b\b\b\b%2d%%\t Tau: %d:\t theta: %d\n', round(ii / (total_iter) * 100), to, theta(ii));
        end
        ...
        relErrTh2=abs(mean(tau(op.burnIn:ii))-mean(tau(op.burnIn:ii-1)))/mean(tau(op.burnIn:ii-1));
        tol_taus(ii) = relErrTh2;
        
        
        
        if ii > op.burnIn 
            %% mean of theta
            mean_thetas(ii- op.burnIn)  = exp(mean(eta(op.burnIn:ii)));
            %% mean tau
            mean_taus(ii - op.burnIn) = mean(tau(op.burnIn:ii));
    ...
    tau_EB = mean(tau(op.burnIn:last_samp));%tau(last_samp);
    results.mean_tau= tau_EB;
    tau_last = tau(last_samp);  
    results.last_tau= tau_last;
    results.taus=tau(1:last_samp);  
    results.mean_taus = mean_taus;
    results.tol_taus = tol_taus;
