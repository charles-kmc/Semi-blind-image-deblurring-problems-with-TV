...
function [dkernel] = diff_fftgaus_w2(im_size, taille,w1,w2,phi)
    ...
    [sum_psf, ~, sum_d_w2] = Sum_gauss_psf(taille, w1, w2,phi);
    
    c = w1^2 * U.^2 + w2^2 * V.^2;
    
    f = ((w1*w2) / (2*pi)) * exp(-c/2);
    
    diff = (w1/(2*pi)) * (1 - w2^2*V.^2) .* exp(-c/2);
    
    dkernel = (diff * sum_psf - f * sum_d_w2)  / (sum_psf^2);
